...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Where(e => e.Mgr.HasValue).Join(emps, e => e.Mgr, m => m.EmpNo, (e, m) => new { Employee = e.EName, Manager = m.EName }).ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { e, d.DName })
            .SelectMany(ed => grades.Where(s => ed.e.Sal >= s.Losal && ed.e.Sal <= s.Hisal), (ed, s) => new { ed.e.EName, ed.DName, s.Grade }).ToList(); 
